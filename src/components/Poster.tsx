...
const Poster = ({ id, title, poster_path, vote_average, release_date, overview }: PosterType) => {
    ...
    return (
        <div className="relative w-56 h-[370px] mb-3 shadow-xl hover:scale-105 transition duration-500" onMouseOver={handleMouseOver} onMouseOut={handleMouseOut}>
            <Link key={id} href={`/movie/${id}`}>
                ...
                <div className={mouseOver ? "" : "opacity-0"}>
                    ...
                    <button className="px-2 py-1 border border-primary-content text-primary-content hover:border-info hover:text-info rounded-md absolute left-2/4 translate-x-[-50%] bottom-24">상세정보</button>
                </div>
            </Link>
        </div>
    )
